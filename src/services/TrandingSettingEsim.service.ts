...
export interface ResponseSettingEsim {
  language: "EN";
  ios: [
    {
      model: null;
...
export type Response = AxiosResponse<ResponseCommon<{ instructions: ResponseSettingEsim }>>;

class TrandingSettingService {
  setting(body: RequestCommon<RequestSettingEsim>): Promise<Response> {
    return httpServices.post(`${ROOT_URL}`, body);
  }
}

export default new TrandingSettingService();
